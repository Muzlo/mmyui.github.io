...
{{this.$route.params.songid}}
        <div class="li" v-for="(item,index) of musicList">
            <nut-row>
                <nut-col :span="5">
                    <div class="flex-content leftPic">
                        <img width="100%" :src="item.songPicSmall" alt="">
                    </div>
                </nut-col>
                <nut-col :span="13">
            ...
            <audio @ended="audioEndFn($event)" class="audio" :src="item.showLink" controls="controls"></audio>
        </div>
...
<script>
    ...
    export default{
        ...
        mounted(){
            ...
            this.axios({
              method: 'post',
              url: 'https://api.apiopen.top/musicDetails',
              data: Qs.stringify(data)
            }).then(response => {
                if(response.data.code==200){
                    if(response.data.result.songList.length){
                        this.musicList=response.data.result.songList
                    }else{
                        this.$router.go(-2);
                        this.$toast.warn('暂无歌曲数据');
                    }
